...
#endif
      /* benchmark: inner loop with 1<<5 calls of aycw_block_sbox()
            gcc mode 1     15,74M
            gcc mode 2     15,12M
            msvc mode 1    13,4M
            msvc mode 2    15,5M

            the whole sbox lookup takes 92%  of the block decrypt function! :-(
            */
}
...
#ifndef USEBLOCKVIRTUALSHIFT
#error all bit slice without virtual shift not implemented!
#endif
